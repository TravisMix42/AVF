...
//Rare exceptions to indicate a bug in the _nativefeatures code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_nativefeatures = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_nativefeatures]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.nativefeatures.module"
