...
NSString * nativeFeatures$ModuleRequireFormat = @"(function(exports){"
...
@implementation nativeFeaturesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_nativefeatures);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create nativeFeatures global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "nativeFeatures" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_nativefeatures = [[nativeFeaturesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_nativefeatures];
	
	NSString *_nativefeaturesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _nativefeaturesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_nativefeatures valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:nativeFeatures$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.nativefeatures.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.nativefeatures.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
