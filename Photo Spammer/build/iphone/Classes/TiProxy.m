...
//Rare exceptions to indicate a bug in the _photospammer code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_photospammer = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_photospammer]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.photospammer.module"
